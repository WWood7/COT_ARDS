...
def CalculateFeatures(train_case_segs, test_case_segs, train_control_segs, test_control_segs,
    ...
    #
    # split the segments into training test and validation test
    # case patients
    print("CV data...")
    kf = KFold(n_splits=fold_num)
    train_case_CVtrainindex_list = []
    train_case_CVvalindex_list = []
    print('case_whole_train:', len(train_case_segs))
    print('case_whole_train subject:', len(train_case_segs['subject_id'].unique()))

    for train_index, val_index in kf.split(train_case_segs['subject_id'].unique()):
        train_case_subid = train_case_segs['subject_id'].unique()[train_index]
        val_case_subid = train_case_segs['subject_id'].unique()[val_index]
        train_case_CVtrainindex_list.append(
            train_case_segs[train_case_segs['subject_id'].isin(train_case_subid)].index)
        train_case_CVvalindex_list.append(
            train_case_segs[train_case_segs['subject_id'].isin(val_case_subid)].index)
    # control patients
    train_control_CVtrainindex_list = []
    train_control_CVvalindex_list = []
    print('control_whole_train:', len(train_control_segs))
    print('control_whole_train subject:', len(train_control_segs['subject_id'].unique()))
    for train_index, val_index in kf.split(train_control_segs['subject_id'].unique()):
        train_control_subid = train_control_segs['subject_id'].unique()[train_index]
        val_control_subid = train_control_segs['subject_id'].unique()[val_index]
        train_control_CVtrainindex_list.append(
            train_control_segs[
                train_control_segs['subject_id'].isin(train_control_subid)].index)
        train_control_CVvalindex_list.append(
            train_control_segs[
                train_control_segs['subject_id'].isin(val_control_subid)].index)

    #
    # calculate features
    # save the results
    cv_folder_path = direction + '/l1trend_features/cross_validation'
    if not os.path.exists(cv_folder_path):
        os.makedirs(cv_folder_path)

    print("Calculating CV features...")
    for i in range(len(vital_list)):
        ...
        for j in range(len(lambda_list)):
            ...
            for indexToFold in range(fold_num):
                ...
                print(train_case_CVtrainindex_list[indexToFold])
                case_train, case_val = train_case_segs.loc[
                                           train_case_CVtrainindex_list[indexToFold]], \
                                       train_case_segs.loc[
                                           train_case_CVvalindex_list[indexToFold]]
                print(len(case_train['hadm_id'].unique()))
                # get the imputed vital signs for case and control patients for this fold
                imputed_vital_train_case = pd.merge(vital, case_train.loc[:, ['icustay_id']], how="inner",
                                                    on=["icustay_id"]).drop_duplicates(keep='first')
                imputed_vital_val_case = pd.merge(vital, case_val.loc[:, ['icustay_id']], how="inner",
                                                  on=["icustay_id"]).drop_duplicates(keep='first')
                control_train, control_val = train_control_segs.loc[
                                                 train_control_CVtrainindex_list[indexToFold]], \
                                             train_control_segs.loc[
                                                 train_control_CVvalindex_list[indexToFold]]
                imputed_vital_train_control = pd.merge(vital, control_train.loc[:, ['icustay_id']], how="inner",
                                                       on=["icustay_id"]).drop_duplicates(keep='first')
                imputed_vital_val_control = pd.merge(vital, control_val.loc[:, ['icustay_id']], how="inner",
                                                     on=["icustay_id"]).drop_duplicates(keep='first')

                train_case_feature_table = case_get_trend_feature(imputed_vital_train_case, case_train,
                                                                  vital_name,
                                                                  duration, delta_value, method)

                train_case_feature_table.to_csv(
                    cv_folder_path + '/l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_train_case_fold' + str(
                        indexToFold) + '.csv')
                val_case_feature_table = case_get_trend_feature(imputed_vital_val_case, case_val, vital_name,
                                                                duration,
                                                                delta_value, method)
                val_case_feature_table.to_csv(
                    cv_folder_path + '/l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_val_case_fold' + str(
                        indexToFold) + '.csv')
                train_control_feature_table = control_get_trend_feature(imputed_vital_train_control,
                                                                        control_train,
                                                                        vital_name, duration, delta_value)
                train_control_feature_table.to_csv(
                    cv_folder_path + '/l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_train_control_fold' + str(indexToFold) + '.csv')
                val_control_feature_table = control_get_trend_feature(imputed_vital_val_control, control_val,
                                                                      vital_name, duration, delta_value)
                val_control_feature_table.to_csv(
                    cv_folder_path + '/l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_val_control_fold' + str(indexToFold) + '.csv')
...
def formTPFP4EachFeature(direction, predictors):
    ...
    # form tokens CV
    cv_folder_path = direction + 'l1trend_features/cross_validation/'
    tpfp_cv_folder_path = direction + 'l1trend_features/cross_validation/tpfp'
    if not os.path.exists(tpfp_cv_folder_path):
        os.makedirs(tpfp_cv_folder_path)
    for i in range(len(vital_list)):
        vital_name = vital_list[i]
        ...
        for j in range(len(lambda_list)):
            delta_value = lambda_list[j]
            for indexToFold in range(fold_num):
                train_case_feature_table = pd.read_csv(
                    cv_folder_path + 'l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_train_case_fold' + str(
                        indexToFold) + '.csv')[predictors]
                ...
                train_control_feature_table = \
                    pd.read_csv(cv_folder_path + 'l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_train_control_fold' + str(indexToFold) + '.csv')[predictors]
                ...
                print(len(train_case_feature_table) + len(train_control_feature_table), len(train_feature_table),
                      "should be the same")
                val_case_feature_table = pd.read_csv(
                    cv_folder_path + 'l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_test_case_fold' + str(
                        indexToFold) + '.csv')[predictors]
                ...
                for feature in predictors:
                    ...
                    control_num = len(train_control_feature_table)
                    temp_TPR_max_upper = 0
                    temp_TPR_max_lower = 0
                    for k in range(len(unique_list)):
                        # print(k)
                        FPR_upper = len(train_control_feature_table.loc[
                                            train_control_feature_table[feature] >= temp_upper]) / control_num
                        FPR_lower = len(train_control_feature_table.loc[
                                            train_control_feature_table[feature] <= temp_lower]) / control_num
                        if FPR_upper <= FPR_max:
                            TPR_upper = len(
                                train_case_feature_table[train_case_feature_table[feature] >= temp_upper]) / case_num
                            if TPR_upper >= temp_TPR_max_upper:
                                temp_TPR_max_upper = TPR_upper
                                temp_threshold_upper = temp_upper
                        if FPR_lower <= FPR_max:
                            TPR_lower = len(
                                train_case_feature_table[train_case_feature_table[feature] <= temp_lower]) / case_num
                            if TPR_lower >= temp_TPR_max_lower:
                                temp_TPR_max_lower = TPR_lower
                                temp_threshold_lower = temp_lower
                        temp_upper = unique_list[len(unique_list) - 1 - k]
                        temp_lower = unique_list[k]
                ...
                temp.to_csv(tpfp_cv_folder_path + '/l1trendfeature_' + str(
                    delta_value) + '_' + vital_name + '_fold' + str(
                    indexToFold) + '_TPFPs.csv')
                print(TPR_max_upper, threshold_upper)
                print(TPR_max_lower, threshold_lower)

    #
    # form tokens for the whole training set
    tpfp_folder_path = direction + 'l1trend_features/tpfp/'
    for i in range(len(vital_list)):
        vital_name = vital_list[i]
        print("Forming " + vital_name + "...")
        for j in range(len(lambda_list)):
            ...
            train_case_feature_table = \
                pd.read_csv(
                    direction + 'feature_tables/' + 'l1trendfeature_' + str(
                        delta_value) + '_' + vital_name + '_train_case.csv')[
                    predictors]
            train_case_feature_table = train_case_feature_table.loc[
                train_case_feature_table['slope_pos_duration_percent'] != -1]
            ...
            for feature in predictors:
                ...
                for k in range(len(unique_list)):
                    # print(k)
                    FPR_upper = len(
                        train_control_feature_table[train_control_feature_table[feature] >= temp_upper]) / non_case_num
                    FPR_lower = len(
                        train_control_feature_table[train_control_feature_table[feature] <= temp_lower]) / non_case_num
                    if FPR_upper <= FPR_max:
                        TPR_upper = len(
                            train_case_feature_table[train_case_feature_table[feature] >= temp_upper]) / case_num
                        if TPR_upper >= temp_TPR_max_upper:
            ...
            temp = pd.DataFrame(
                {'predictors': predictors, 'TPR_max_upper': TPR_max_upper, 'threshold_upper': threshold_upper,
                 'TPR_max_lower': TPR_max_lower, 'threshold_lower': threshold_lower})
            temp.to_csv(tpfp_folder_path + 'l1trendfeature_' + str(
...
def pickup_top40token4EachVital(generate_path, predictors):
    ...
    # pick up tokens
    # form CV
    cv_folder_path = generate_path + 'l1trend_features/cross_validation/top_features'
    if not os.path.exists(cv_folder_path):
        os.makedirs(cv_folder_path)
    for i in range(len(vital_list)):
        vital_name = vital_list[i]
        ...
        for indexToFold in range(fold_num):
            ...
            for k in predictors:
                ...
                for j in range(len(delta_value_list)):
                    ...
                    TF_temp = pd.read_csv(
                        generate_path + 'l1trend_features/cross_validation/tpfp/' + 'l1trendfeature_' + str(
                            delta_value) + '_' + vital_name + '_fold' + str(
                            indexToFold) + '_TPFPs.csv')
            ...
            vital_top20.to_csv(
                cv_folder_path + '/l1trendfeature_' + vital_name + '_fold' + str(
                    indexToFold) + '_TOP40.csv')
            print(vital_top20)

        # combine the CV TOP 40
        folder_path = generate_path + 'l1trend_features/top_features'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        vital_top20_list = []
        for indexToFold in range(fold_num):
            vital_top20 = pd.read_csv(
                cv_folder_path + '/l1trendfeature_' + vital_name + '_fold' + str(
                    indexToFold) + '_TOP40.csv').reset_index(drop=True)
            vital_top20['strlabel'] = pd.DataFrame({'strlabel': [str(val) for val in vital_top20['label']]})
            vital_top20['predictors_label'] = vital_top20['predictors'] + vital_top20['strlabel']
            vital_top20_list.append(vital_top20)
        vital_top20_allcv = pd.concat(vital_top20_list)
        vital_top20_final_list = []
        for pl in vital_top20_allcv['predictors_label'].unique():
            pl_temp = vital_top20_allcv[vital_top20_allcv['predictors_label'] == pl].nlargest(1, 'TPR_max')
            vital_top20_final_list.append(pl_temp)
        vital_top20_final_temp = pd.concat(vital_top20_final_list).reset_index(drop=True)

        # vital_top20_final = vital_top20_final_temp.nlargest(20, 'TPR_max')
        vital_top20_final_temp.to_csv(
            folder_path + '/' + vital_name + '_TOP40.csv')


def Getabnomral_labs_wLH(path):
    data_path = path + 'data/'
    generate_path = path + 'generate/lab/'
    ...
    norm_labevents = labevents[labevents['FLAG'] != 'abnormal']
    # here use the lab events with normal flags to calculate a mean
    ab_labevents_id['Mean'] = np.nan
    for itemid in ab_labevents_id['ITEMID'].unique():
        onelab_mean = norm_labevents['VALUE'][norm_labevents['ITEMID'] == itemid].mean()
        # print(norm_labevents['VALUE'][norm_labevents['ITEMID']==itemid].head())
        # print(onelab_mean)
    ...
    for key in merge_labs.keys():
        ...
        for i in range(len(merge_labs[key]) - 1):
            if len(ab_labevents_id[ab_labevents_id['ITEMID'] == merge_labs[key][i + 1]]) != 0:
                temp_mean = ab_labevents_id['Mean'][ab_labevents_id['ITEMID'] == merge_labs[key][i + 1]].unique()[0]
                print(temp_mean)
                if temp_mean != np.nan:
                    print("In")
                    merge_mean_count = merge_mean_count + 1
                    merge_mean = temp_mean + merge_mean
                ab_labevents_id['ITEMID'][ab_labevents_id['ITEMID'] == merge_labs[key][i + 1]] = merge_labs[key][0]
        if merge_mean_count != 0:
            ab_labevents_id['Mean'][ab_labevents_id['ITEMID'] == merge_labs[key][0]] = merge_mean / merge_mean_count
    print(len(ab_labevents_id['ITEMID'].unique()))

    # 把abnormal拿出来
    ab_labevents = labevents[labevents['FLAG'] == 'abnormal']
    ab_labevents_wLabel = pd.merge(
        ab_labevents, d_labitems[['ITEMID', 'LABEL']],
        how="left", on=["ITEMID"]).drop_duplicates(keep='first')
    for key in merge_labs.keys():
        for i in range(len(merge_labs[key]) - 1):
            ab_labevents_wLabel['ITEMID'][ab_labevents_wLabel['ITEMID'] == merge_labs[key][i + 1]] = merge_labs[key][0]
        ab_labevents_wLabel['LABEL'][ab_labevents_wLabel['ITEMID'] == merge_labs[key][0]] = key
    ...
    for itemid in ab_labevents_wLabelMean['ITEMID'].unique():
        count = count + 1
        print("COUNT!!!!!!!!!!!!!!!!!!!!!!!!!:", count)
        if np.isnan(ab_labevents_wLabelMean['Mean'][ab_labevents_wLabelMean['ITEMID'] == itemid].unique()[0]):
            print(itemid, ab_labevents_wLabelMean['LABEL'][ab_labevents_wLabelMean['ITEMID'] == itemid].unique()[0])
        else:
            r_count = r_count + 1
            range_mean = ab_labevents_wLabelMean['Mean'][ab_labevents_wLabelMean['ITEMID'] == itemid].unique()[0]
            print(range_mean)
            ab_high = ab_labevents_wLabelMean[
                (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] >= range_mean) & (
                        ab_labevents_wLabelMean['FLAG'] == 'abnormal')]
            if len(ab_high) != 0:
                ab_labevents_wLabelMean['LH'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] >= range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')] = 'High'
                temp1 = ab_labevents_wLabelMean['ITEMID'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] >= range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')].unique()[0]
                temp2 = ab_labevents_wLabelMean['LABEL'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] >= range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')].unique()[0]
                # print(temp1, temp2)
                ab_labevents_wLabelMean['ITEMID'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] >= range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')] = temp1 + 2
                ab_labevents_wLabelMean['LABEL'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] >= range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')] = temp2 + "_High"
            ab_low = ab_labevents_wLabelMean[(ab_labevents_wLabelMean['ITEMID'] == itemid) & (
                    ab_labevents_wLabelMean['VALUE'] < range_mean) & (ab_labevents_wLabelMean['FLAG'] == 'abnormal')]
            if len(ab_low) != 0:
                ab_labevents_wLabelMean['LH'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] < range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')] = 'Low'
                temp3 = ab_labevents_wLabelMean['ITEMID'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] < range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')].unique()[0]
                temp4 = ab_labevents_wLabelMean['LABEL'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] < range_mean) & (
                            ab_labevents_wLabelMean['FLAG'] == 'abnormal')].unique()[0]
                ab_labevents_wLabelMean['ITEMID'][
                    (ab_labevents_wLabelMean['ITEMID'] == itemid) & (ab_labevents_wLabelMean['VALUE'] < range_mean) & (
    ...
    print(len(ab_labevents_wLabelMean['ITEMID'].unique()))
    ab_labevents_wLabelMean.to_csv(generate_path + 'abnormal_labs_wLH.csv')
    return ab_labevents_wLabelMean


def GetLabTokens(train_segs, test_segs, abnormal_labs_wLH):
    ...
    train_case = train_segs['case'][['hadm_id', 'icustay_id', 'segend', 'segstart', 'starttime']]
    train_control = train_segs['control'][
        ['hadm_id', 'icustay_id', 'segend', 'seg_id', 'segstart', 'starttime']]
    test_case = test_segs['case'][['hadm_id', 'icustay_id', 'segend', 'segstart', 'starttime']]
    test_control = test_segs['control'][
        ['hadm_id', 'icustay_id', 'segend', 'seg_id', 'segstart', 'starttime']]
    ...
    for id in range(len(uniperIDs)):
        datahadmID.append(train_case_lab[train_case_lab['icustay_id'] == uniperIDs[id]])
    for i in range(len(datahadmID)):
        ...
        if len(datahadmID[i]) > 0:
            ...
            for j in datahadmID[i]['ITEMID'].unique():
                ...
                # print(temp_onelab[['hadm_id', 'CHARTTIME', 'VALUE', 'VALUEUOM', 'ITEMID', 'LABEL']].iloc[-1]['hadm_id'])
...
def GetVentTokens(train_segs, test_segs, abnormal_vent, vent_token_method):
...
def GetDemoTokens(train_segs, test_segs, demographics):
    train_case = train_segs['case'][['hadm_id', 'icustay_id', 'segend', 'segstart']]
    train_control = train_segs['control'][['hadm_id', 'icustay_id', 'segend', 'segstart']]
    test_case = test_segs['case'][['hadm_id', 'icustay_id', 'segend', 'segstart']]
    test_control = test_segs['control'][['hadm_id', 'icustay_id', 'segend', 'segstart']]
...
def formattedVitalTokenInput(file_path, fold_num):
    vital_list = ['heartrate', 'sysbp', 'meanbp', 'spo2', 'tempc', 'resprate']
    # form final vital inputs
    # 从对应的feature的table里产生token和对应的time,id, value, time以onset为0，前几小时就为几，要把train和test的都生成好
    # from input token CV
    for indexToFold in range(fold_num):
        vital_digit = 0
        train_case_vital_token = []
        train_control_vital_token = []
        val_case_vital_token = []
        val_control_vital_token = []
        # test_case_vital_token = []
        # test_control_vital_token = []
        for vital_name in vital_list:
            vital_digit = vital_digit + 1
            vital_top20 = pd.read_csv(
                file_path + 'generate/tokens/vital/' + vital_name + '_TOP40.csv')
            for i in range(len(vital_top20)):
                delta = format(vital_top20['delta'][i], '.1g')
                predictor = vital_top20['predictors'][i]
                value = vital_top20['value'][i]
                label = vital_top20['label'][i]

                train_case_feature = pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_train_case_fold' + str(
                        indexToFold) + '.csv')[['icustay_id', vital_top20['predictors'][i]]]
                val_case_feature = pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_val_case_fold' + str(
                        indexToFold) + '.csv')[['icustay_id', vital_top20['predictors'][i]]]
                train_control_feature = pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_train_control_fold' + str(
                        indexToFold) + '.csv')[['seg_id', vital_top20['predictors'][i]]]
                val_control_feature = pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_val_control_fold' + str(
                        indexToFold) + '.csv')[['seg_id', vital_top20['predictors'][i]]]
                if label == 1:
                    train_case_token = train_case_feature[train_case_feature[predictor] >= value]
                    ...
                    val_case_token = val_case_feature[val_case_feature[predictor] >= value]
                    val_case_token['time'] = 0
                    val_case_token['token'] = predictor + '_' + str(label)
                    val_case_token['token_id'] = 10000 + vital_digit * 1000 + i + 1
                    val_case_token = val_case_token.rename(columns={"icustay_id": "PatientID", predictor: "value"})
                    ...
                    train_control_token = train_control_token.rename(
                        columns={"seg_id": "PatientID", predictor: "value"})
                    val_control_token = val_control_feature[val_control_feature[predictor] >= value]
                    val_control_token['time'] = 0
                    val_control_token['token'] = predictor + '_' + str(label)
                    val_control_token['token_id'] = 10000 + vital_digit * 1000 + i + 1
                    val_control_token = val_control_token.rename(columns={"seg_id": "PatientID", predictor: "value"})
                else:
                    train_case_token = train_case_feature[train_case_feature[predictor] <= value]
                    train_case_token['time'] = 0
                    train_case_token['token'] = predictor + '_' + str(label)
                ...
                val_case_vital_token.append(val_case_token)
                val_control_vital_token.append(val_control_token)
        ...
        val_case_vital_token_input = pd.concat([obj for obj in val_case_vital_token])
        val_control_vital_token_input = pd.concat([obj for obj in val_control_vital_token])
        train_case_vital_token_input.to_csv(
            file_path + 'generate/tokens/vital/folds/train_case_fold' + str(
                indexToFold) + '_vital_token_input.csv')
        train_control_vital_token_input.to_csv(
            file_path + 'generate/tokens/vital/folds/train_control_fold' + str(
                indexToFold) + '_vital_token_input.csv')
        val_case_vital_token_input.to_csv(
            file_path + 'generate/tokens/vital/folds/val_case_fold' + str(
                indexToFold) + '_vital_token_input.csv')
        val_control_vital_token_input.to_csv(
            file_path + 'generate/tokens/vital/folds/val_control_fold' + str(
                indexToFold) + '_vital_token_input.csv')

    # from input token whole train
    vital_digit = 0
    train_case_vital_token = []
    train_control_vital_token = []
    test_case_vital_token = []
    ...
    for vital_name in vital_list:
        ...
        for i in range(len(vital_top40)):
            delta = format(vital_top40['delta'][i], '.1g')
            predictor = vital_top40['predictors'][i]
            value = vital_top40['value'][i]
            label = vital_top40['label'][i]
            test_case_feature = \
                pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_test_case.csv')[
                    ['icustay_id', vital_top40['predictors'][i]]]
            test_control_feature = \
                pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_test_control.csv')[
                    ['seg_id', vital_top40['predictors'][i]]]
            train_case_feature = \
                pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_train_case.csv')[
                    ['icustay_id', vital_top40['predictors'][i]]]
            train_control_feature = \
                pd.read_csv(
                    file_path + 'generate/feature_tables/l1trendfeature_' + delta + '_' + vital_name + '_train_control.csv')[
                    ['seg_id', vital_top40['predictors'][i]]]
            if label == 1:
                train_case_token = train_case_feature[train_case_feature[predictor] >= value]
                train_case_token['time'] = 0
    ...
    train_case_vital_token_input.to_csv(
        file_path + 'generate/tokens/vital/train_case_vital_token_input.csv')
    train_control_vital_token_input.to_csv(
        file_path + 'generate/tokens/vital/train_control_vital_token_input.csv')
    test_case_vital_token_input.to_csv(
        file_path + 'generate/tokens/vital/test_case_vital_token_input.csv')
    test_control_vital_token_input.to_csv(
        file_path + 'generate/tokens/vital/test_control_vital_token_input.csv')
