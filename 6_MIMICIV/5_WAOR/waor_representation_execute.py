...
optFea_WAOR_list = {'a': [1],
                    'b': [2]}  # {'gama': 0.5, 'alpha': 1./2, 'beta':0.1}#
combination = []
...
for iscontrol in [0, 1]:
    ...
    hitstruct = np.load(
        tokenarray_path + caseorcontrol + '_HitArray_dict_' + str(FPR_max)
        + '_sparse.npy', allow_pickle=True).item()
    patientid = list(hitstruct.keys())
    toolbox_input = np.load(
        tokenarray_path + caseorcontrol + '_toolbox_input_' + str(FPR_max)
